...
def _request(
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=_timeout()) as resp:
            ...
            return SmokeResult(name=path, passed=ok, status_code=code, detail=body[:200])
...
def run_smoke_tests() -> list[SmokeResult]:
    ...
    # Public health (no API prefix on root health)
    base = os.environ.get("API_BASE_URL", "http://localhost:8000").rstrip("/")
    for path in ("/health", "/health/auth", "/health/ai"):
        try:
            ...
            with urllib.request.urlopen(req, timeout=_timeout()) as resp:
                results.append(
                    SmokeResult(name=path, passed=resp.status == 200, status_code=resp.status)
                )
        except Exception as exc:
            results.append(SmokeResult(name=path, passed=False, detail=str(exc)))
    ...
    if student:
        results.append(
            _request("/production/readiness", token=student, expect_status=403)
        )
        results.append(
            _request("/security/summary", token=student, expect_status=403)
        )
